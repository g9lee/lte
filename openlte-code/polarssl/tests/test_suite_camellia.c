...
int verify_int( char *str, int *value )
{
    ...
    if( strcmp( str, "POLARSSL_ERR_CAMELLIA_INVALID_INPUT_LENGTH" ) == 0 )
    {
        *value = ( POLARSSL_ERR_CAMELLIA_INVALID_INPUT_LENGTH );
        return( 0 );
    }
#endif // POLARSSL_CIPHER_MODE_CBC
    if( strcmp( str, "POLARSSL_ERR_CAMELLIA_INVALID_KEY_LENGTH" ) == 0 )
    {
        *value = ( POLARSSL_ERR_CAMELLIA_INVALID_KEY_LENGTH );
        return( 0 );
    }
...
int main()
{
    ...
    const char *filename = "suites/test_suite_camellia.data";
    FILE *file;
    char buf[5000];
    char *params[50];
